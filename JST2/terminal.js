...
function block_img(link) {
    current_block.innerHTML += "<img src='" + link + "'></img>";
}

function img(link) {
    var wrapper = document.getElementById('wrapper');
    current_block.innerHTML += "<div class='log'><img src='" + link + "'></img></div>";
}
...
function block_pde(message) {
    current_block.innerHTML += "<pde>" + message + "</pde>";
}

function pde(message) {
    var wrapper = document.getElementById('wrapper');
    wrapper.innerHTML += "<div class='log'><pde>" + message + "</pde></div>";
}
...
function submit_command() {
    ...
    if (registry.has(command.split(" ")[0].toUpperCase())) {
        ...
        block_log("Error: '" + command.split(" ")[0].toUpperCase() + "' is not defined");
    }
}
